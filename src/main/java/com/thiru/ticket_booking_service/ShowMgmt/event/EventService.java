...
import com.thiru.ticket_booking_service.ShowMgmt.event.DTO.*;
...
public class EventService implements AuthorizationPolicy<EventEntity, UserEntity> {
    ...
    public void create(CreateEvent event) {
        UserEntity admin = userRepo.findByName(event.getUserName())
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.NOT_FOUND,
                        "User not found: " + event.getUserName()));
        this.canCreate(admin);
        event.setEventType(event.getEventType().toUpperCase());
        event.setEventTypeEnum(EventType.valueOf(event.getEventType()));
    ...
    public void update(UpdateEvent event) {
        ...
        if (event.getEventTypeEnum() != null)
            existing.setEventType(event.getEventTypeEnum());
    ...
    public List<EventReadResponse> read(ReadEvent request) {
        ...
        if (request.getEventId() != null) {
            ...
            UserEntity admin = userRepo.findByName(request.getUserName())
                    .orElseThrow(() -> new ResponseStatusException(
                            HttpStatus.NOT_FOUND,
                            "User not found: " + request.getUserName()));
            events = eventRepo.findByUserId(admin.getUserId());
        } else if (request.getEventName() != null && request.getEventTypeEnum() != null) {
            events = eventRepo.findByEventNameAndEventType(request.getEventName(), request.getEventTypeEnum());
        } else if (request.getEventName() != null) {
            events = eventRepo.findByEventName(request.getEventName());
        } else if (request.getEventTypeEnum() != null) {
            events = eventRepo.findByEventType(request.getEventTypeEnum());
        }
